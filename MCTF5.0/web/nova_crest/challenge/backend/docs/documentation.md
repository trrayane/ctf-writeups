...
2. Compile the AST into query plans
3. Execute query plans against registered Mongoose models

High-level flow:

```text
Source -> parse() -> AST -> compile() -> QueryPlan[] -> execute
```

## Error Handling

`parse()` throws `QMongoParseError` for syntax errors and includes:
